...
あいさつ・食べ物・動物・花・旅行・地名 の 6 カテゴリ、全 62 フレーズ（各 10 件以上）。

- 切り替えは**起動画面のチップ**と **⚙️ 設定**の両方から。練習中は画面を一切占有しないので、モバイルでも縦の領域を圧迫しません
- 選んだカテゴリは `localStorage` に保存され、次回もそのカテゴリで始まります
- カテゴリ名は 11 言語すべてに翻訳済みです
- 画像のファイル名一覧は [images-needed.tsv](images-needed.tsv) にあります（未アップロードの画像はプレースホルダに差し替わります）
...
```
index.html      画面の構造
images-needed.tsv  フレーズごとに必要な画像ファイル名の一覧
